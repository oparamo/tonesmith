...
const cn1 = basePatch("CN-1 VERSE", "FX1>AMP>FX2>NS>DLY>REV");
amp(cn1, "BRIT STACK", 18, 50, 48, 58, '4x12"', "RIBON121");
fx(cn1, "fx1", "CHORUS", "STEREO", { rate: 28, depth: 55, level: 70, pre_delay: 4 });
fx(cn1, "fx2", "SCRIPT PH", null, { rate: 15, depth: 40, level: 55 });
clearOdds(cn1);
ns(cn1, 25, 35);
delay(cn1, "ANALOG", 1, 32, 45, "2.5kHz");
reverb(cn1, "HALL M", 3.5, 42, 30, -8, 5, 100);
patches.push(cn1);

// CN-2  Solo 1 — FX1(Comp) → OD(Muff) → AMP → NS → DLY → REV
const cn2 = basePatch("CN-2 SOLO1", "FX1>OD>AMP>NS>DLY>REV");
...
clearOdds(wywh1);
ns(wywh1, 25, 35);
delay(wywh1, "ANALOG", 1, 25, 38, "2.5kHz");
reverb(wywh1, "HALL M", 3.0, 35, 28, -8, 4, 100);
patches.push(wywh1);

// WYWH-2  Strummed Rhythm
const wywh2 = basePatch("WYWH-2 RHYTH", "FX1>AMP>FX2>NS>DLY>REV");
...
ns(wywh3, 35, 50);
delay(wywh3, "ANALOG", 1, 33, 52, "3.15kHz");
reverb(wywh3, "HALL M", 3.0, 40, 18, -5, 6, 100);
patches.push(wywh3);

// HAC-1  Have A Cigar Rhythm — T-SCREAM boost, high gain
const hac1 = basePatch("HAC-1 RHYTHM", "FX1>OD>AMP>NS>DLY>REV");
...
ns(hac1, 45, 28);
delay(hac1, "ANALOG", 1, 22, 35, "3.15kHz");
reverb(hac1, "PLATE", 1.6, 28, 10, 5, 6, 100);
patches.push(hac1);

// HAC-2  Have A Cigar Lead — 60S FUZZ
const hac2 = basePatch("HAC-2 LEAD", "FX1>OD>AMP>NS>DLY>REV");
...
ns(hac2, 42, 45);
delay(hac2, "ANALOG", 1, 28, 48, "3.15kHz");
reverb(hac2, "PLATE", 1.8, 35, 12, 5, 7, 100);
patches.push(hac2);

// HY-1  Hey You Rhythm — clean
const hy1 = basePatch("HY-1 RHYTHM", "FX1>AMP>FX2>NS>DLY>REV");
amp(hy1, "DELUXE", 15, 55, 50, 52, '1x12"', "RIBON121");
fx(hy1, "fx1", "COMPRESSOR", "ORANGE", { sustain: 40, attack: 60, level: 58 });
fx(hy1, "fx2", "CHORUS", "STEREO", { rate: 12, depth: 20, level: 50, pre_delay: 3 });
clearOdds(hy1);
ns(hy1, 30, 40);
delay(hy1, "ANALOG", 1, 28, 42, "2.5kHz");
reverb(hy1, "HALL M", 2.8, 38, 25, -10, 5, 100);
patches.push(hy1);

// HY-2  Hey You Lead
const hy2 = basePatch("HY-2 LEAD", "FX1>OD>AMP>NS>DLY>REV");
...
ns(hy2, 40, 55);
delay(hy2, "ANALOG", 1, 38, 55, "3.15kHz");
reverb(hy2, "HALL S", 3.2, 42, 20, -5, 6, 100);
patches.push(hy2);

// ABW-1  Another Brick Rhythm — T-SCREAM, very high gain
const abw1 = basePatch("ABW-1 RHYTHM", "FX1>OD>AMP>NS>DLY>REV");
amp(abw1, "BRIT STACK", 60, 50, 40, 70, '4x12"', "DYN57");
fx(abw1, "fx1", "CHORUS", "STEREO", { rate: 30, depth: 40, level: 60, pre_delay: 0 });
odds(abw1, "T-SCREAM", 20, 10, 70);
ns(abw1, 55, 30);
delay(abw1, "STANDARD", 7, 28, 45, "FLAT");
reverb(abw1, "PLATE", 1.8, 30, 0, 0, 5, 100);
patches.push(abw1);

// TD-1  Time/Dogs Lead — 60S FUZZ
const td1 = basePatch("TD-1 LEAD", "FX1>OD>AMP>FX2>NS>DLY>REV");
...
ns(td1, 35, 45);
delay(td1, "ANALOG", 1, 32, 50, "3.15kHz");
reverb(td1, "HALL S", 2.0, 38, 15, 0, 5, 100);
patches.push(td1);

// ECH-1  Echoes Clean Verse
const ech1 = basePatch("ECH-1 CLEAN", "FX1>AMP>FX2>NS>DLY>REV");
amp(ech1, "BRIT STACK", 20, 52, 45, 65, '4x12"', "DYN421");
fx(ech1, "fx1", "CHORUS", "STEREO", { rate: 22, depth: 45, level: 65, pre_delay: 3 });
fx(ech1, "fx2", "SCRIPT PH", null, { rate: 16, depth: 38, level: 52 });
clearOdds(ech1);
ns(ech1, 30, 38);
delay(ech1, "ANALOG", 1, 30, 45, "2.5kHz");
reverb(ech1, "HALL S", 2.5, 36, 18, -5, 5, 100);
patches.push(ech1);

// ECH-2  Echoes Chaos — extreme settings
const ech2 = basePatch("ECH-2 CHAOS", "FX1>OD>AMP>FX2>NS>DLY>REV");
...
ns(ech2, 0, 0, false);
delay(ech2, "ANALOG", 1, 55, 65, "4kHz");
reverb(ech2, "HALL S", 4.5, 55, 10, 5, 8, 100);
patches.push(ech2);

// ECH-3  Echoes Main Lead — 60S FUZZ
const ech3 = basePatch("ECH-3 LEAD", "FX1>OD>AMP>FX2>NS>DLY>REV");
...
ns(ech3, 36, 48);
delay(ech3, "ANALOG", 1, 32, 50, "3.15kHz");
reverb(ech3, "HALL S", 2.8, 38, 16, -3, 5, 100);
patches.push(ech3);

// BRE-1  Breathe Rhythm
const bre1 = basePatch("BRE-1 RHYTHM", "FX1>AMP>FX2>NS>DLY>REV");
...
clearOdds(bre1);
ns(bre1, 28, 42);
delay(bre1, "ANALOG", 1, 30, 40, "2kHz");
reverb(bre1, "HALL M", 3.2, 40, 28, -10, 4, 100);
patches.push(bre1);

// BRE-2  Breathe Lead
const bre2 = basePatch("BRE-2 LEAD", "FX1>OD>AMP>FX2>NS>DLY>REV");
...
ns(bre2, 35, 48);
delay(bre2, "ANALOG", 1, 30, 48, "3.15kHz");
reverb(bre2, "HALL M", 3.0, 38, 18, -5, 5, 100);
patches.push(bre2);

// MAR-1  Marooned Slide Rhythm — MODULATE delay, SHIMMER reverb
const mar1 = basePatch("MAR-1 RHYTH", "FX1>AMP>FX2>NS>DLY>REV");
amp(mar1, "TWIN", 16, 50, 52, 60, "ORIGINAL", "CND87");
fx(mar1, "fx1", "CHORUS", "STEREO", { rate: 14, depth: 38, level: 60, pre_delay: 4 });
fx(mar1, "fx2", "TREMOLO", null, { rate: 22, depth: 30, level: 65 });
clearOdds(mar1);
ns(mar1, 22, 65);
delay(mar1, "MODULATE", 1, 38, 50, "2.5kHz", true, { mod_rate: 12, mod_depth: 18 });
reverb(mar1, "SHIMMER", 4.0, 48, 30, -5, 5, 100, true, { pitch: 12, pitch_lvl: 25 });
patches.push(mar1);

// MAR-2  Marooned Main Slide Lead
const mar2 = basePatch("MAR-2 LEAD", "FX1>OD>AMP>FX2>NS>DLY>REV");
...
ns(mar2, 28, 60);
delay(mar2, "MODULATE", 1, 40, 55, "3.15kHz", true, { mod_rate: 10, mod_depth: 15 });
