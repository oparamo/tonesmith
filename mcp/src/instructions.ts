...
Building patches takes two lookups and one build, however many patches you are building:

1. describe_device <device> with no \`items\` lists this device's groups and every type id in them.
   That is the vocabulary step 2 names, so skip this call only when you already know the ids.
2. describe_device <device> items: [...] is ONE call naming "chain" plus every type you need,
   written "<group>/<type>". "chain" returns the device's block order, how bypass works, and the
   settings the patch carries itself rather than inside any block (a reference tempo, an output
   trim), which you set beside \`name\`. Each "<group>/<type>" entry returns that type's params with
   their exact key, range, and allowed values, plus an \`example\`: that block's spec at factory
   defaults, which you copy into the call below and change the values you care about. A bare group
   id returns an index of what the group holds rather than any params, so name the types. Work the
   full list out first, rather than one call per effect. This is the only source for a device's
   blocks, types and params; no tool schema repeats them.
3. generate_patch takes every patch in the \`patches\` array, in the order you want them on the
