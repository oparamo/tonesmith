...
The GX-1 exposes overdrive in two places: the dedicated `odds` block, and the `OD/DS` entry in the
...
dedicated block carries the same models and is where a patch's overdrive belongs; the `odds` group
